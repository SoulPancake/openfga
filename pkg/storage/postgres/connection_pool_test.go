...
// Connection pool tests for PostgreSQL datastore to ensure that the following environment variables
...
func TestPostgresConnectionPoolMaxOpenConnections(t *testing.T) {
	...
	for i := 0; i < numOperations; i++ {
		go func() {
			...
			rows, err := ds.primaryDB.QueryContext(ctx, "SELECT pg_sleep(0.1)")
			if err != nil {
				errors <- err
				return
			}
	...
	// Final stats check
	finalStats := ds.primaryDB.Stats()
	require.LessOrEqual(t, finalStats.OpenConnections, maxOpenConns)
}

func TestPostgresConnectionPoolMaxIdleConnections(t *testing.T) {
	testDatastore := storagefixtures.RunDatastoreTestContainer(t, "postgres")
	uri := testDatastore.GetConnectionURI(true)
	...
	for i := 0; i < 3; i++ {
		rows, err := ds.primaryDB.QueryContext(ctx, "SELECT 1")
		require.NoError(t, err)
		rows.Close()
	}
...
func TestPostgresConnectionPoolMaxLifetime(t *testing.T) {
	testDatastore := storagefixtures.RunDatastoreTestContainer(t, "postgres")
	uri := testDatastore.GetConnectionURI(true)
	...
	initialStats := ds.primaryDB.Stats()
...
func TestPostgresConnectionPoolMaxIdleTime(t *testing.T) {
	testDatastore := storagefixtures.RunDatastoreTestContainer(t, "postgres")
	uri := testDatastore.GetConnectionURI(true)
...
func TestPostgresConnectionPoolLifetimeLimits(t *testing.T) {
	testDatastore := storagefixtures.RunDatastoreTestContainer(t, "postgres")
	uri := testDatastore.GetConnectionURI(true)
	...
	start := time.Now()
	rows, err := ds.primaryDB.QueryContext(ctx, "SELECT pg_sleep(0.3)")
	require.NoError(t, err)
	rows.Close()
